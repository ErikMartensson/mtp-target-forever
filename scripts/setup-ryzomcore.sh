...
cmake -S "$RYZOMCORE_DIR" -B "$BUILD_DIR" -G Ninja \
    ...
    -DWITH_NEL=ON -DWITH_SOUND=ON -DWITH_GUI=OFF \
...
echo ""
echo "=== Building NeL libraries and drivers ==="
ninja -C "$BUILD_DIR" \
    nelmisc nelnet nel3d nelsound nelsnd_lowlevel nelgeorges nelligo nelpacs \
    nel_drv_opengl nel_drv_openal

echo ""
echo "=== Verifying build ==="
missing=0
for lib in libnelmisc.a libnelnet.a libnel3d.a libnelsound.a libnelsnd_lowlevel.a \
           libnelgeorges.a libnelligo.a libnel_drv_opengl.so libnel_drv_openal.so; do
